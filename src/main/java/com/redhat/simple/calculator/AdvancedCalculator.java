...
public final class AdvancedCalculator extends Calculator {
    ...
    public double calculateDiscountForLowPurchaseAmount(
            final boolean isBusiness, final int yearsAsCustomer) {
        if (isBusiness) {
    ...
    public double calculateDiscountForHighPurchaseAmount(
            final boolean isBusiness, final int yearsAsCustomer) {
        if (isBusiness) {
            return VIP_DISCOUNT_AMOUNT;
        }
        return calculateHighPurchaseDiscountBasedOnCustomerYears(
                yearsAsCustomer);
    }

    public double calculateHighPurchaseDiscountBasedOnCustomerYears(
...
//        double discount = 0;
        ...
        if (saleAmount < LOW_PURCHASE_AMOUNT) {
            return calculateDiscountForLowPurchaseAmount(
                    isBusiness, yearsAsCustomer);
        }
        if (saleAmount > HIGH_PURCHASE_AMOUNT) {
            return calculateDiscountForHighPurchaseAmount(
                    isBusiness, yearsAsCustomer);
        }
